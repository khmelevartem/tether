...
A folder or multi-file send opens with `POST /batch-begin`, body `{"batchId","totalFiles","totalBytes"}` (totalBytes null when unknown; `400 {"error":"invalid_batch"}` when totalFiles < 1). The receiver registers the batch against the sending peer (resolved by remote address, as for `/upload`), then accepts one `POST /upload` per file. The batchId scopes one send attempt: a new id resets the peer's receive counter; a repeat is idempotent. A sender that skips `/batch-begin` is treated as a one-file implicit batch.

The batch closes one of three ways. It completes on its own when the receiver's per-file count reaches totalFiles. If the sender delivered fewer files than declared (sources it could not read), it posts `POST /batch-end` and the receiver finalizes the shortfall as a partial. If the sender deliberately cancels, it posts `POST /batch-cancel` and the receiver records a sender-cancelled partial rather than a network drop. Both carry only `{"batchId"}`, answer `200 {}`, and are idempotent — an unknown, stale, or already-completed batchId is ignored; a dropped connection needs neither call, as the receiver terminates through the per-file failure path when its reconnect window expires. `/batch-end` is retried by the sender with exponential backoff to survive a transient drop of that one request; `/batch-cancel` stays best-effort.

### Known limitation

The receiver finalizes a batch only on a real signal: per-file count reaching the declared total, a delivered `/batch-end`, or a socket-level connection drop (which starts the reconnect countdown above). It deliberately does not guess-finalize on inbound silence — the wire gives it no per-chunk signal to distinguish a sender still uploading a large or slowly-materializing file from a sender that is gone, so a silence timer false-positives on exactly the transfers most worth protecting. The one case this leaves unhandled: the sender crashes or the network dies *between* files, after a `FileCompleted` but before `/batch-end` succeeds. There is no open socket to time out and no completion signal, so the receiver's card stays in the receiving state until the user cancels it.
