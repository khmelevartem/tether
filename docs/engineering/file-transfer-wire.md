...
A folder or multi-file send opens with `POST /batch-begin`, body `{"batchId","totalFiles","totalBytes"}` (totalBytes null when unknown). The receiver registers the batch against the sending peer (resolved by remote address, as for `/upload`) and answers `200 {}`, or `400 {"error":"invalid_batch"}` when totalFiles < 1. The batchId scopes one send attempt: a begin with a new id resets the peer's receive counter and totals; a repeat of the same id is idempotent. The receiver completes the batch as soon as its per-file completion count reaches totalFiles. A sender that posts to `/upload` without a preceding `/batch-begin` is treated as a one-file implicit batch.

When the sender finishes walking the source list, it posts `POST /batch-end`, body `{"batchId"}`. This finalizes a batch that delivered fewer files than declared — the sender skipped sources it could not read, so the receiver's completion count never reaches totalFiles and would otherwise wait forever. The receiver answers `200 {}` and records the shortfall as an unreadable-files partial. The call is best-effort and idempotent: on a batch that already completed at its full count it is a no-op, and an unknown or stale batchId is silently ignored. It is *not* sent when the batch is cancelled (that path uses `/batch-cancel`) or lost to an unrecovered network drop (the receiver times out on its own) — only on a normal end-of-list completion.
