...
- **One layer per package.** Package boundaries mark the canonical layers even inside a single module: domain types and interfaces, data-layer implementations, presentation components, and composables each live under their own package. Per-layer ownership and import rules: [layering.md](layering.md).
