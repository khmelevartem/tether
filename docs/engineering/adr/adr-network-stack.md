...
|---|---|---|---|---|---|
| All five targets covered with one code path | ✅ | ❌ (two server engines) | ❌ (two server stacks) | ✅ | ✅ |
...
| TLS upgrade path ([#140](https://github.com/khmelevartem/tether/issues/140)) | ✅ engine handles it | ✅ Netty's TLS is more mature; CIO Native unchanged | ⚠️ Apple-side TLS to wire ourselves (Network.framework gives it) | ❌ we wire TLS | ❌ we wire TLS |
...
**Stay on Ktor CIO for both client and server across all targets (Option 1).** The premise of the original quick-research choice holds up under the deeper review: every alternative either splits the stack along JVM/Native lines (Option 2 and 3), trades engine maturity for parser-maintenance work (Option 4), or loses HTTP tooling for marginal gain (Option 5). The pain points that triggered this ADR are tuning gaps and a few CIO bugs, not architectural failures of the engine choice.
...
3. **CIO Native iOS server is published-and-tested but not officially listed in the Native server targets doc** ([server-native.html](https://ktor.io/docs/server-native.html) names macOS / Linux / Windows native, not iOS). Our [appleTest/FileServerTest.kt](../../../composeApp/src/appleTest/kotlin/com/tubetoast/tether/network/FileServerTest.kt) is the safety net. Revisit trigger below.
...
- **TLS / channel encryption design ([#140](https://github.com/khmelevartem/tether/issues/140)) selects a path that needs `Network.framework`-specific features** (e.g. native TLS session caching, PSK). Re-evaluate Apple-side server engine then.
