...
2. **One mental model, two actuals.** The JVM and Apple `FileServer.actual`s now read the same: `embeddedServer(CIO, port) { routing { get("/health"); post("/upload"); post("/batch-begin"); post("/batch-end"); post("/batch-cancel") } }`. Reviewers and future contributors don't context-switch between two server architectures when changing the protocol.
