...
- [File transfer wire contract](file-transfer-wire.md) — `POST /upload?name=<relative-path>` per-file transfer with batch framing, two-layer path sanitization (lexical in route + canonical-realisation in `UploadStorage`), per-platform storage seam.
