...
- **Should the app surface that the device is currently sharing Wi-Fi?** A subtle indicator ("You're sharing Wi-Fi — other devices can find you here") could help the host understand why their phone is suddenly visible to a stranger's Tether install. Could equally be a noisy mode-indicator that nobody reads. Decide after first user observations.
  - **Forward hook — share-network entry point.** If the indicator earns its keep, it is the natural anchor for a future "share this hotspot" action: tap → show a QR / printable creds for guests to join the hotspot in one scan, instead of dictating the password. That feature is out of scope here and gets its own scoping when (and if) the indicator ships. The decision now is only whether to reserve the surface; the action lights up later.
