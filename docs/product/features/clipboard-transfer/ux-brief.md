...
- **Contract-sync note.** This extension to the offline-paired (Case 3) row is recorded in device-list's own contract — [device-list/spec.md §Row contract](../device-list/spec.md#row-contract) and [device-list/ux-brief.md Case 3](../device-list/ux-brief.md) — both flagging it as owned by this feature and linking back here, so the docs do not drift.
