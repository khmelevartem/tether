...
- **macOS-host Downloads TCC prompt** (runtime, macOS 10.15+). On the first incoming transfer, the OS prompts: "Tether would like to access files in your Downloads folder." Triggered by the OS, not by Tether. Unlike the sandboxed native build, the unsandboxed JVM build cannot opt into this via entitlement — the OS governs unsandboxed access to Downloads/Documents/Desktop/iCloud/removable volumes. Tether shows its rationale screen immediately before the first incoming transfer is accepted. On denial, the receive cannot complete (no fallback save path); Tether shows a help screen pointing at Privacy & Security → Files and Folders. Retry on next foreground.
