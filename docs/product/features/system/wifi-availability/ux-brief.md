...
- Illustration region: the **disconnected-state illustration**. Static, no animation. Visually distinct from the searching indicator.
...
- Same disconnected-state illustration.
...
3. User sees: disconnected-state illustration, "Wi-Fi is off", rationale, and — on Android — the "Open Wi-Fi settings" button.
...
6. Within a few seconds, DeviceListScreen transitions to the searching state (animated searching indicator).
...
2. **Disconnected-state illustration** — a static illustration distinct from the searching indicator. Used exclusively in the no-local-network screen.
