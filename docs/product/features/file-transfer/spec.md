...
- **Share-sheet send is a handoff, not a synchronous selection.** An iOS share extension is a separate process and cannot drop files into a running Tether in one step, so sharing into Tether does not land the user directly at the device list. The extension stages the shared files and surfaces a way back into Tether (opening it, or a tap-through notification); Tether ingests them on next foreground and presents them as a pending selection, from which the user continues to the device list. The "files already selected at the device list" flow above is the post-handoff end state. A selection consumed but not yet sent does not survive a cold relaunch — the originals stay in the source app, so the user re-shares.
