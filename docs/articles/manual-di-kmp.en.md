...
The key idea is KMP-specific and structural: the container hierarchy mirrors the source-set hierarchy. A component's place follows from which API its source set can see. I haven't seen this framed in the manual-DI writing out there: it shows up as a stepping stone to a framework, or described outside any KMP context.

I'm not rejecting frameworks on principle. A framework solves real problems, and on a large project it pays for itself. But it also brings a compiler plugin or KSP, its own build lifecycle, its own rules, and its own learning curve. Until the problems it solves show up, it's more apparatus than the task deserves. This article walks through how manual DI works in [Tether](https://github.com/khmelevartem/tether), a KMP file-transfer app for Android, iOS, and Desktop, and where the line sits past which a framework earns its place.
...
I'll cover each, first at the app level, then how the same approach scales to standalone KMP libraries.
...
Each component receives its dependencies through the constructor. The container knows the assembly order and owns the singletons. `SyncService` doesn't know where `MessageRepository` came from; it accepts it. That's dependency injection: the caller passes it in.

The payoff shows up in tests. Because `SyncService` takes `MessageRepository` through the constructor, a test passes a fake there: no mocks, no reflection, no swapping globals.

### This is DI, not a service locator

A common objection to manual DI: in practice it degrades into a service locator, a global registry that classes reach into for their dependencies. The objection misses. The difference is the direction of control. With a service locator the class knows about the registry and pulls dependencies from it. With constructor injection the class knows of no container at all; the composition root passes dependencies into it. The container is where assembly happens, and classes never read from it. [Martin Fowler](https://martinfowler.com/articles/injection.html) drew this line in detail, and it separates real DI from its imitation.

The one place a class does reach for the container itself is Android components with empty constructors. That's an explicit, fenced concession, covered below.

## The container hierarchy mirrors the source-set hierarchy

A KMP app has a tree of containers. The tree mirrors the source-set tree. Each layer adds only what its source set can see.
...
The boundary runs along API visibility. If a component is built from shared abstractions, it lives in the common container. If it needs a platform type, it drops into the platform leaf. You construct a component where the required API first becomes visible.
...
Config is the input. The container is what gets assembled from it. The split gives you one place to set parameters and another to build the graph from them.

Concrete config implementations are named classes in their own files, not anonymous `object : AppConfig { … }` literals. An anonymous object at the call site invites drift between assembly points: two places construct an "almost identical" config and drift apart. A named class is the single source of truth.

## Platform entry points

Each platform builds its own container leaf in its own entry point and hands components down.

- On **Android**, you build the container in the `Application` subclass; it lives as long as the process.
- On **iOS**, you build the container in the UI entry point, but outside the `Composable` lambda, so the composable doesn't become a composition root (more on this below).
...
Android comes with a specific complication. The framework constructs its managed components (`Activity`, `Service`, `BroadcastReceiver`), and they must have empty constructors. You can't pass them the container through the constructor like a normal class.
...
This is not a service locator, even though it looks like one. The difference is discipline: the provider interface is implemented by exactly one type (the production `Application`, and a test one in tests), and only Android-framework-managed classes reach the container through it. Plain Kotlin classes still receive their dependencies through the constructor. Static access is an explicit, unavoidable concession to empty constructors, nothing more.

The pattern itself is Android's own official recommendation for manual DI ([Manual dependency injection](https://developer.android.com/training/dependency-injection/manual)). The docs frame it as a stepping stone to Hilt; here it stays a standalone solution until the triggers in the final section fire.
...
**One singleton, one owner.** If a component is meant to be a singleton, the composition root constructs it once. Not in an `object`, not from deep inside another component. The root is the only thing that knows lifecycles; everywhere else you receive what's already alive.
...
`DisposableEffect(Unit)` runs once and captures the instance from the first composition. Every later recomposition creates a new `SomeService` and drops it at once, never started, never stopped. `remember` fixes the leak but leaves the real mistake: the composable owns a singleton's lifecycle, which it shouldn't. The correct shape is to build the container in the entry point and pass the ready component in.
...
A test builds a fake container by subclassing and overriding the fields it needs. You test a component that takes its dependencies through the constructor on its own; it doesn't need the container, which exists for production wiring.

To test Android components under Robolectric you need an `Application` that implements the provider interface. Either the production `Application`, or a test one with a fake container. Each test gets a fresh instance, nothing settles into a static.
...
Why the cut: it strictly marks the library's public API and, as a result, thins the integration layer. A consumer sees what the library promised and can't reach an implementation detail by accident.
...
From there, you pass the ready container into consumers' constructors:
...
Manual DI pays off up to a certain scale. Past that scale it hurts, and moving to a framework becomes a deliberate choice.
...
- **support for every target platform**: iOS targets, JVM, Android all supported;
...
- **The structure tells you where to construct things.** The container tree mirrors the source-set tree, and a component's place follows from where the required API first becomes visible.
- **A thin integration layer for libraries.** The Api/Impl split strictly marks the public surface.
- **A cheap move to a framework.** When the triggers fire, the shape already matches a graph's, so you relocate without a structural reshuffle.

Until the project outgrows it, you have as much DI as you need and not a line more.
