...
- **Дешёвый переход.** Контейнеры и провайдеры по форме уже близки к графу фреймворка — переезжать можно без структурных изменений ([Миграция на фреймворк](#миграция-на-фреймворк)).
...
Объектами, живущими ровно пока открыт экран, владеет слой навигации или экрана — обычным Kotlin: они приходят в конструктор сверху или создаются на месте, а отпускаются вместе с владельцем, когда тот закрывается. Этого хватает, пока владельцев жизненного цикла немного и они выражены явными классами. Когда нужны DI-управляемые скоупы, lifecycle-bound зависимости или выгружаемые фичи — ответ даёт фреймворк (см. [«Миграция на фреймворк»](#миграция-на-фреймворк)).
...
Структурный триггер по сути один — **скоупы и lifecycle-bound зависимости**. Когда графу нужен жизненный цикл короче процесса (зависимость живёт ровно пока открыт экран или сессия и разбирается при закрытии; часть графа выгружается из памяти, когда не нужна), ручной DI упирается: контейнеры — синглтоны на процесс, механизма скоупов у подхода нет. Фреймворк объявляет дочерний граф декларативно и сам разбирает его при закрытии скоупа — это и есть сигнал переходить.

Рядом — оговорка про интеграцию с экосистемой. Когда окружающие фреймворки рассчитаны на конкретный DI (инъекция в Android `ViewModel`, `Worker`, навигацию), собрать это руками — повторяющийся клей под каждую точку. Это накладная стоимость, не предел подхода, и её стоит взвесить.

Пока скоупы не понадобились, фреймворк — лишний аппарат.
