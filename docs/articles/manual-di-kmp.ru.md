...
Промежуточный `jvmMain` как общий родитель `androidMain` и `desktopMain` — это настраиваемая иерархия сорс-сетов, а не форма KMP по умолчанию: в дефолтной структуре Android и JVM лежат соседями под `commonMain` без общего jvm-слоя, и совместный `jvmMain` включается шаблоном иерархии в Gradle.
...
Иерархия контейнеров собирает граф наследованием: лист достраивает то, что общий контейнер уже определил. Композиция дополняет это наследование, а не вытесняет его: платформенный лист по-прежнему наследует общий контейнер и при этом получает в конструктор отдельный фрагмент-ось. Контейнер складывается из отдельных именованных фрагментов, и каждый фрагмент отвечает за одну ось, по которой приложение варьируется. Выделяем такую ось, выносим её в отдельный фрагмент контейнера и подставляем нужный фрагмент при сборке. Осей может быть несколько.
...
- На **iOS** контейнер собирается в точке входа UI, например в `MainViewController`, но снаружи composable-лямбды, которая описывает интерфейс, — чтобы эта лямбда не стала ещё одним composition root.
