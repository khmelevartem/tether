...
Composition root — единственное место, где приложение собирает граф зависимостей. Это место принято называть контейнером. Контейнер создаёт компоненты — рядовые объекты графа, которыми владеет приложение: сервисы, хранилища, клиенты — по одному разу и раздаёт их всем, кому они нужны. Компонент здесь не Gradle-модуль и не сам контейнер, а объект внутри графа. Никто, кроме контейнера, не вызывает конструктор общего компонента.
...
Иерархия контейнеров собирает граф наследованием: лист достраивает то, что общий контейнер уже определил. Композиция — другая возможная форма сборки: контейнер складывается из отдельных именованных фрагментов, и каждый фрагмент отвечает за одну ось, по которой приложение варьируется. Выделяем такую ось, выносим её в отдельный фрагмент контейнера и подставляем нужный фрагмент при сборке. Осей может быть несколько.
...
Учение Симана говорит об одном composition root на приложение. Назвать компонент экрана вторым composition root — значит размыть этот принцип, поэтому отдельного «скоупного корня» в подходе нет.
...
Ещё один размен — между наглядностью и compile-time проверкой графа. Забытую зависимость компилятор и так ловит: конструктор без аргумента не вызвать, поэтому «забыл предоставить X» просто не скомпилируется. Единственная брешь — циклические зависимости через ленивые поля (`by lazy`): когда два компонента ссылаются друг на друга, цикл всплывает в рантайме при первом обращении, а граф-валидирующий фреймворк ловит его на сборке. Брешь узкая и одна; к тому же всплывший цикл — это пойманная реальная проблема, а не спрятанная, так что чистым проигрышем это назвать сложно.
...
Это применимо как к модулям внутри проекта, так и к публикующимся библиотекам. Тот же разрез имеет смысл между обычными модулями внутри одного приложения: как только модуль А не должен опираться на внутренности модуля Б, Б делится на Api и Impl, и на Impl остаётся ровно одна зависимость — из модуля, который собирает граф.
