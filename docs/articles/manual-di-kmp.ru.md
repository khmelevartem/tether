...
- **компоновка через границы модулей превращается в рутину** — модулей много, и вклады из них собираются в контейнер вручную по одному и тому же шаблону.
...
У триггера про границы модулей есть предел. Автоматический сбор вкладов из модулей в общий граф — то, чем фреймворк закрывает эту рутину, — работает только внутри одного приложения: модули делят общий маркер скоупа, и кодогенерация сшивает их на финальной сборке. Опубликованная библиотека сюда не попадает — она компилируется раньше приложения и про его граф не знает, да и не должна: иначе частность оказалась бы привязана к месту, где её применяют. Через эту границу оба подхода связывают одинаково: библиотека отдаёт Api и фабрику, а втягивает её в граф потребитель — у себя в корне, где и остаётся всё знание о месте использования.
