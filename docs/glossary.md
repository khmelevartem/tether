...
- **Pairing trust** — the recognition recorded on a device when pairing completes: the set of peer identities it will accept transfers from without re-prompting. The asset an attacker wants their own key written into. _Avoid:_ trust store when the user-facing relationship is meant (use **Trusted device** for the peer, **Pairing trust** for the relationship). (see [security.md](product/security.md))
- **PIN** — the 4-digit numeric code both devices display during pairing, derived from the handshake, that the user compares across both screens to confirm they are pairing with each other and not an interposed attacker. _Avoid:_ passcode, password, OTP. (see [security.md §Pairing Flow](product/security.md#pairing-flow))
- **Consent gate** — the step that withholds pairing trust until both users independently confirm the matching PIN; a pairing request alone grants nothing. _Avoid:_ confirmation dialog, approval prompt. (see [security.md](product/security.md))
- **Presence disclosure** — the inherent visibility, on a shared LAN, that a person runs Tether, on what device type, under what display name, and which peers they pair with. Treated as observable by design rather than a separately defended asset. _Avoid:_ social-graph leak, metadata leak. (see [security.md](product/security.md))
...
- **Key pinning** — verifying a peer's presented certificate against the specific public key stored for it at pairing, never against the OS trust store; the **pinned key** is that stored public half. A peer whose key is not pinned fails the TLS handshake. _Avoid:_ certificate pinning (the pin is on the public key, not the certificate), CA validation. (see [adr-channel-encryption.md](engineering/adr/adr-channel-encryption.md))
