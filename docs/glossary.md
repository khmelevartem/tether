...
- **Atomic destination reservation** — the step that picks a deduplicated leaf name and atomically creates a zero-byte placeholder file before any byte of the upload body arrives, so two concurrent uploads with the same name cannot resolve to the same path. _Avoid:_ destination claim, path lock. (see [file-transfer-wire.md](engineering/file-transfer-wire.md))
