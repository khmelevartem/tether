...
- **Repository** — a presentation-layer-adjacent state container that owns per-key (e.g., per-peer) mutable state and exposes `observe` + command methods to Components. Distinct from the DDD data-access pattern of the same name: Tether Repositories hold in-memory UI state, not persistent data.
