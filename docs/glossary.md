...
- **Review burden** — the reviewer effort a PR drew, measured from its ROOT review comments and review rounds; the ground-truth signal for issue sizing, independent of code volume. _Avoid:_ review load, review cost. (see [sizing-rubric.md](../.claude/sizing-rubric.md))
- **Review rounds** — the count of distinct review sessions on a PR: ROOT review comments clustered by `created_at`, a gap over 60 minutes opening a new cluster. _Avoid:_ review iterations, commit count. (see [sizing-rubric.md](../.claude/sizing-rubric.md))
- **ROOT review comment** — a top-level inline PR review comment (`in_reply_to == null`) — the reviewer's substantive feedback point, as opposed to a thread reply (often a `/check-review` auto-acknowledgement). _Avoid:_ top comment, first comment. (see [sizing-rubric.md](../.claude/sizing-rubric.md))
