...
- **QEMU user-mode NAT** — the default network back-end (SLIRP) used by the Android emulator; presents the guest behind a virtual firewall that blocks unsolicited inbound connections. (see [android-emulator-networking.md](knowledge/android-emulator-networking.md))
- **Re-announcement** — periodic re-broadcast of mDNS service records by the publisher to refresh cached entries on resolvers before TTL expiry.
- **TXT record** — a DNS resource record carrying arbitrary key=value metadata for a service; in DNS-SD the wire format is a sequence of length-prefixed strings (RFC 6763 §6.1).
