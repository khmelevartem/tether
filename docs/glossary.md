...
- **ADR** — Architecture Decision Record. A `docs/engineering/adr/adr-<name>.md` artifact recording one-time architectural choices (context, options, decision, consequences) and append-only; threshold and conventions in [adr/README.md](engineering/adr/README.md). _Avoid:_ design doc, architecture note.
- **Revisit-if trigger** — a condition listed in an ADR's `Revisit if` section that, when it fires (an accepted cost turns out to be blocking, a constraint behind the original choice has changed), forces re-evaluating the decision (confirm with new evidence or reverse per [adr/README.md](engineering/adr/README.md) §Reversing an ADR). _Avoid:_ Revisit-if event when the activation aspect is what matters.
