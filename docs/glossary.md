...
- **Review rounds** — the count of distinct review sessions on a PR: ROOT review comments clustered by time, a gap over 60 minutes opening a new cluster. _Avoid:_ review iterations, commit count. (see [sizing-rubric.md](../.claude/sizing-rubric.md))
- **ROOT review comment** — a top-level inline PR review comment — the reviewer's substantive feedback point, as opposed to a thread reply (often a `/check-review` auto-acknowledgement). _Avoid:_ top comment, first comment. (see [sizing-rubric.md](../.claude/sizing-rubric.md))
