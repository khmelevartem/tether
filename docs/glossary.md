...
- **SAS** — Short Authentication String: the short human-readable value each device derives independently from the full agreed key material at pairing and shows the user to compare by eye. A matching SAS on both screens confirms no MITM substituted keys. _Avoid:_ PIN, 4-digit code, pairing code, verification code. (see [threat-model.md §SAS pairing model](engineering/threat-model.md#sas-pairing-model))
- **Commit-before-reveal** — the SAS-pairing step where each device sends a hash commitment of its public key before the peer reveals its key, so an active MITM cannot fit its key to the victim's SAS after seeing the victim's key. _Avoid:_ commitment scheme (too generic here). (see [threat-model.md §SAS pairing model](engineering/threat-model.md#sas-pairing-model))
