...
- **UploadHandle** — a value returned by `UploadStorage.resolveDestination` carrying the reserved destination path and the list of parent directories created during the call, used to scope `abort` to only the directories this upload owns.
- **Atomic destination reservation** — the step inside `UploadStorage.resolveDestination` that picks a deduplicated leaf name and atomically creates a zero-byte placeholder file before any byte of the upload body arrives, so two concurrent uploads with the same `name` cannot resolve to the same path. Implemented per-platform (`File.createNewFile` on JVM, `fopen("wbx")` on Apple) behind the `UploadStorageBackend` interface, with `JvmUploadStorageBackend` / `AppleUploadStorageBackend` per platform. _Avoid:_ destination claim, path lock. (see [file-transfer-wire.md](engineering/file-transfer-wire.md))
...
- **Abort** — UploadStorage's failure-path cleanup: deletes the partial destination file and removes only the empty parent directories the upload created (those recorded in the UploadHandle). _Avoid:_ cancel, rollback.
