...
- **Repository** — a domain-layer mediator between higher-level logic and one or more data sources, following standard Clean Architecture / DDD usage. Owns per-key state aggregation and exposes `observe` + command methods to consumers. Holds no transport or persistence detail itself; those live behind a **DataSource**. (see [file-transfer.md](engineering/file-transfer.md))
- **DataSource** — the seam a Repository depends on for inbound events and outbound dispatch. Production implementations adapt transport (`FileClient` / `FileServer`) or storage; tests inject a fake. _Avoid:_ backend, provider, gateway. (see [file-transfer.md](engineering/file-transfer.md))
