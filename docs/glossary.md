...
- **CLI identity (ephemeral / persistent)** — the Desktop CLI's device identity (name + fingerprint + key pair) is per-process ephemeral by default so co-located instances stay distinct; `--config-dir` opts into a persistent identity stored in that directory, stable across restart. (see [cli-multi-instance.md](knowledge/cli-multi-instance.md))
