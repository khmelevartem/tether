...
- **EC P-256** — the NIST P-256 elliptic curve (also called prime256v1 or secp256r1); the asymmetric key type Tether uses for device identity. Each device generates one P-256 keypair on first launch and retains it for its lifetime.
- **Keychain** — the platform-managed secure store for cryptographic keys; on Apple platforms, accessed via Security framework APIs. Tether's `KeychainStore` seam abstracts it for testability. _Avoid:_ keystore (Android term), secure enclave (a sub-component of the Keychain stack, not the Keychain itself).
- **uncompressed point** — the 65-byte encoding of an EC public key: `0x04 || X || Y`, where X and Y are the 32-byte big-endian coordinates. Apple's `SecKeyCopyExternalRepresentation` returns this form for P-256 keys.
- **X.509 SPKI** — SubjectPublicKeyInfo, a 91-byte DER structure wrapping an uncompressed EC point with an AlgorithmIdentifier header. The format the JVM's `KeyFactory("EC").generatePublic(X509EncodedKeySpec(...))` accepts and what Tether exchanges in the pairing handshake.
