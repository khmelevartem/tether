...
- **Trust store** — the persistent per-device store of public keys for every device that completed pairing; implemented as `TrustedDeviceStore`. A peer whose key is present is a **Trusted device** and may transfer without re-pairing. _Avoid:_ keystore (Android term), trusted list.
