...
// Pure helpers extracted out of the JNA-driven Session so they can be unit-tested
// without loading libSystem.
internal object BonjourCodec {
    // Convert a host-byte-order port (e.g. 8080) to the network-byte-order Short
    // expected by DNSServiceRegister's `port` parameter on macOS.
    fun hostOrderToNetwork(port: Int): Short =
        ((((port and 0xFF) shl 8) or ((port ushr 8) and 0xFF))).toShort()

    // Convert the network-byte-order port reported by DNSServiceResolve back to a
    // host-byte-order Int. Symmetric to hostOrderToNetwork.
    ...
    // Read an IPv4 dotted-quad from a BSD sockaddr_in pointer:
    //   { u8 sa_len, u8 sa_family, u16 sin_port, u8 sin_addr[4], ... }
    // Returns null when the pointer is null or the family byte is not BSD AF_INET (2),
    // so callers can quietly skip IPv6 reports.
