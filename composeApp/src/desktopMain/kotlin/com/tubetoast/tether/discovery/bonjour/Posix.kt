...
// poll(2) on macOS via libSystem. Used by MdnsDiscoveryBonjour to wait for a
// DNS-SD socket to be readable with a bounded timeout, so the polling coroutine
// can exit on cancellation without leaving DNSServiceProcessResult blocked
// across a DNSServiceRefDeallocate call (Apple's dns_sd.h forbids that race).
internal interface Posix : Library {
    fun poll(fds: Memory, nfds: Int, timeout: Int): Int

    companion object {
        // pollfd { int fd; short events; short revents; } — 8 bytes on macOS.
