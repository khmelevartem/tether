...
**Conceptual duplication — a new type naming a category the domain already models.** Grepping by name and body misses the case where the diff adds a new enum, sealed hierarchy, or data class that classifies a real-world category the domain layer already models under a different name and a different value set. The tell: a classification type sitting in a presentation or UI package, enumerating real-world categories — a device kind, a peer status, a transfer phase — that the domain layer is the natural owner of. For each such new type, state in one phrase the question it answers, then search the domain packages for an existing type that already answers it — by meaning, not by the new type's name. If one exists, flag it: the new type either reuses the existing one or states why the existing type is unfit for the call site. A divergent value set is not by itself that justification; it is more often the symptom of the duplication.
