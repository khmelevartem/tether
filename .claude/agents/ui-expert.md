...
**Verbatim means character-for-character.** Em-dash (`—`) is not a hyphen. A contraction (`it's`) is not its expansion. A placeholder count of three (`tap <peer> again`, `<peer>'s card`, `tap <peer> again`) is not one. Punctuation, capitalisation, em-dash vs hyphen, ASCII `x` vs `×`, straight vs curly quotes — all preserved. Substitute placeholders (`<peer>`, `<N>`, `<size>`); everything else copies as-is. Paraphrasing produces strings that read plausible and fail UX-conformance review.
