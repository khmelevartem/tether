...
### 2.1 Прогнать smoke-test (если применимо)

Если задача затрагивает рантайм-поведение (Affected area: Desktop CLI, Android, iOS, mDNS, FileServer, сборка, нативные таргеты) — **до того как просить пользователя про ручные проверки** запусти `/smoke-test` сам и приложи verdict (🟢/🟡/🔴) к подтверждению на следующем под-шаге.

Если задача DOCS-only / комментарии / правки в `docs/`, `.claude/`, README — smoke не нужен, пропускай этот под-шаг.
...
(Если на 2.1 был прогон — приложи verdict в этой же реплике.)

**Это жёсткий stop-point.** Не переходи к шагу 3, пока пользователь не ответил явным «ОК / прошли / да». Свой собственный отчёт об удачном smoke не считается за подтверждение — он только повышает уверенность пользователя при ответе.
