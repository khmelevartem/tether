...
The other reviewers (`review-correctness`, `review-tests`, `review-platform`, `review-ui`, `review-ux`, `review-architecture`) **do not run** — there is no code, no UI implementation to check against the brief. UX-brief structural completeness is covered by `review-guides` (it knows the routing `ux-brief.md → ux-expert.md §Output`).
