...
`review-visual` is the most expensive reviewer — a full Roborazzi render runs ~30 minutes. Dispatch it once on the authoritative final diff, not per iteration. Do not re-dispatch it for a change whose visual effect is determinable from code reasoning (e.g. a single-child `Column(CenterHorizontally)` → `Box(contentAlignment = Center)` swap is visually identical) — reason from the source instead. When a layout / centring bug is found in one of several sibling composables (top bars, rows, cards), fix all siblings in the same pass so one render covers them rather than triggering another slow round. If the user offers to eyeball the visuals, take it and skip the agent.
