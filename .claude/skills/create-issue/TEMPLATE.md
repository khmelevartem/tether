...
- **`size:`** — `size:S` ≤ 4 h, `size:M` ≤ 1 day, `size:L` ≤ 3 days. Larger → epic with sub-issues; raise during interview. Size is review effort, not diff size — estimate per [SKILL.md §Estimating size](SKILL.md) / [`sizing-rubric.md`](../../sizing-rubric.md).
