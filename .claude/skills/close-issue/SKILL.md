...
**Before merging: reconcile the `size:*` label against the actual review burden.** Size means human review effort, not diff size — the rubric is [`.claude/sizing-rubric.md`](../../sizing-rubric.md). Read the ground-truth signal off this PR: the count of your ROOT (top-level) inline review comments and the number of review rounds they fall into (as the rubric defines them). Match against the calibration bands; a `size:S` that drew double-digit comments over several rounds is a `size:L`, and vice versa. Update via `gh issue edit <N> --remove-label size:S --add-label size:L`. Do **not** size by lines changed or commit count — a large but mechanical diff stays small, and atomic commits make commit count meaningless. The label must reflect actual effort, otherwise the analytics and the calibration bands `grooming` re-derives drift.
