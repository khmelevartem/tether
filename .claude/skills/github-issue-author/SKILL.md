...
- **Терминология проекта** — если в задаче упоминаются платформы, модули, доменные сущности или другие категории с неоднозначным неймингом, и ты не нашёл явный гайдлайн в `CLAUDE.md` / `AGENTS.md` / `docs/`, спроси пользователя один раз: «Как ты называешь X в этом проекте?» — одной фразой. Пропусти вопрос если терминология явно зафиксирована в репозитории, либо если запрос пользователя уже использует её последовательно. Инженерные имена из репо и продуктовые имена для пользователей часто расходятся — issue читается обоими, и неверный выбор разводит код и продуктовую поверхность.
...
> 2. В `AGENTS.md` встретил `SearchService` и `search-api` — как ты называешь этот модуль в продуктовом контексте, чтобы я использовал то же имя в описании?
> 3. Есть ли parent issue / epic, к которому привязать?
> 4. Какие labels поставить? (или ничего)
