...
Issue читается двумя аудиториями — инженером, который будет его делать, и пользователем/менеджером, который думает в продуктовых терминах. У одной и той же сущности часто два имени: инженерное (из исходников / build system / source set'ов) и продуктовое (из vision / audience / маркетинга). Пример формы рассогласования: source set называется `desktopMain` (JVM-таргет), продукт говорит «Windows и Linux» — issue про «Linux support» и issue про «`desktopMain` cleanup» — это разные задачи, и смешивать имена нельзя.

Перед черновиком собери термины, которые понадобятся для этого issue:

1. **Сканируй явные источники** в порядке приоритета: `docs/product/tech-stack.md`, `docs/product/audience.md`, `docs/product/vision.md`, любой `glossary.md` / `ubiquitous-language.md`, `CLAUDE.md` / `AGENTS.md`, заголовки и таблицы в `docs/engineering/README.md`. Эти источники — формальный словарь проекта.
2. **Определи регистр для issue.** Чисто инженерные задачи (REFACTOR / INFRA внутри одного модуля, build-системные правки) — инженерный регистр. Всё, что описывает поведение продукта (FEATURE / BUGFIX user-visible / DOCS про продукт) — продуктовый. Сомневаешься — продуктовый по умолчанию, потому что issue читают шире, чем PR.
3. **Если для нужной категории термина два правильных варианта и оба встречаются в репо** (или сам пользователь использует оба) — задай **один** вопрос: «Как ты называешь X в этом issue — `<инж-вариант>` или `<продуктовый вариант>`?». Один вопрос на категорию, не на каждое упоминание.
4. **Если категория явно зафиксирована в источниках** и/или запрос пользователя использует её последовательно — не спрашивай, используй найденный термин и упомяни источник в чате одной строкой («использую `Windows / Linux` per `tech-stack.md`»).

В черновике используй выбранный термин **последовательно** — в заголовке, контексте, DoD, out-of-scope. Микс инженерных и продуктовых имён в одном issue — баг.
...
- **Терминология** — выбранные термины для этой задачи (см. подраздел [Терминология проекта](#терминология-проекта) выше). Закрыто на этапе разведки; в интервью попадает только если осталась нерешённая ambiguity по одной-двум категориям.
...
>
> По терминологии: в `AGENTS.md` модуль зовётся `SearchService`, в `docs/product/` — «поиск товаров». Issue про поведение для пользователя, поэтому возьму продуктовое имя — ок?
>
> Уточни, пожалуйста:
