...
Формулировки в стиле «можно проверить да/нет», и **в терминах наблюдаемого поведения или измеримой характеристики**, не структурного артефакта. Класс / интерфейс / файл / метод существуют как способ доставить поведение — но сам по себе их факт существования ничего не доказывает: рядом может лежать пустая реализация, не подключённая в DI, или код, не покрытый ни одним вызывающим путём. Описывай то, что увидит пользователь или автотест.

✗ `DeviceNameStore с реализациями на 4 платформах поднят` — это how, и зелёный чекмарк не означает что store кто-то читает.
✓ `Сохранённое имя устройства переживает перезапуск приложения на всех 4 платформах` — testable, наблюдаемо.

✗ `Discovery.republish(name) добавлен в контракт` — структурный артефакт.
✓ `После изменения имени соседние устройства видят новое имя в течение нескольких секунд` — наблюдаемо снаружи.
