...
- **Don't assert incidental format width** — pin only the bytes that are the invariant under test, not a length a legitimate change can vary. Check that a TXT record carries the `fp=` field, not its exact byte count (the fingerprint length is not the invariant); the `/pair` 91-byte / `0x30` / `0x04` shape *is* the invariant, so it stays pinned. A block FAIL right after a deliberate format or behaviour change is drift, not flakiness — investigate the change before dismissing it.
