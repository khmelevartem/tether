...
# block-4-android self-skips without a device; block-5-ios and block-5.5 do not, so guard both.
if command -v xcodebuild >/dev/null 2>&1 \
  ...
  run BLOCK5.5-iOS-Receive ./block-5.5-ios-receive.sh
else
  run BLOCK5-iOS echo "SKIP: iOS — Xcode or simulator '${IOS_DEVICE:-iPhone 17}' unavailable"
  run BLOCK5.5-iOS-Receive echo "SKIP: iOS receive — Xcode or simulator '${IOS_DEVICE:-iPhone 17}' unavailable"
fi
