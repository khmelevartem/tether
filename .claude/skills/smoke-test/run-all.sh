...
# Cleanup (block-8) runs unconditionally via the EXIT/INT/TERM trap. A watchdog aborts the run
...
run BLOCK1-Desktop-UI-Startup ./block-1-desktop-ui-startup.sh

run BLOCK2   ./block-2-desktop-cli-a.sh
run BLOCK3.1 ./block-3.1-single-file-send.sh
run BLOCK3.2 ./block-3.2-multi-file-send.sh
run BLOCK3.3 ./block-3.3-retry.sh
run BLOCK4   ./block-4-same-name-discovery.sh
run BLOCK4.1 ./block-4.1-peer-dedup.sh
run BLOCK4.2 ./block-4.2-same-name-distinct.sh
run BLOCK4.5 ./block-4.5-rename.sh
run BLOCK5-Android ./block-5-android.sh

# block-5-android self-skips without a device; block-6.1-ios and block-6.2 do not, so guard both.
if command -v xcodebuild >/dev/null 2>&1 \
   && xcrun simctl list devices available 2>/dev/null | grep -q "${IOS_DEVICE:-iPhone 17}"; then
  run BLOCK6.1-iOS ./block-6.1-ios.sh
  run BLOCK6.2-iOS-Receive ./block-6.2-ios-receive.sh
else
  run BLOCK6.1-iOS echo "SKIP: iOS — Xcode or simulator '${IOS_DEVICE:-iPhone 17}' unavailable"
  run BLOCK6.2-iOS-Receive echo "SKIP: iOS receive — Xcode or simulator '${IOS_DEVICE:-iPhone 17}' unavailable"
fi

run BLOCK7 ./block-7-graceful-quit.sh
# block-8 runs via the EXIT trap.
