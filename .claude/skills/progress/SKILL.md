...
- `base(size)` — `S`/`M`/`L` come from [`.claude/sizing-bands.json`](../../sizing-bands.json) (mean review burden per size, regenerated by [`review-burden.py`](../../scripts/review-burden.py) at grooming); `unlabeled`/`retro` from `assets/palette.json#valor_size`. Mapped via the `size:` label on the issue closed by the PR. The label is human review effort, not diff size (rubric: [`sizing-rubric.md`](../../sizing-rubric.md)); the `comments` / `LOC` / `cycleHours` terms below add raw-volume nuance on top.
