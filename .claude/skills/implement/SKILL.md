...
- **G2. BUGFIX root cause** — root cause must be confirmed before any fix. **Mitigation:** dispatch `bug-reproducer`; only stop at user if it reports CANNOT REPRODUCE or none of the listed hypotheses match. **The reproducer must always attempt to observe the symptom**, even when the cause looks structurally evident from issue text + code grep. Code-evident cause says «if X happens, Y is plausible»; it does not prove that Y actually manifests on our hardware. CANNOT REPRODUCE → escalate to user with the explicit framing: «возможно баг заведён теоретически (нет наблюдённого симптома). Продолжать как defensive fix, переклассифицировать как REFACTOR, или закрыть?» — do not silently proceed as if the bug were confirmed.
...
**G2 handling.** If BUGFIX → dispatch `bug-reproducer`. It reproduces locally, verifies each hypothesis, and returns a confirmed cause as structured paste-ready text. It does NOT post to GitHub. **Always dispatch `bug-reproducer`, even when the cause looks structurally obvious from issue + code grep** — the reproducer's job is to confirm the *symptom*, not just the cause. A BUGFIX issue may have been filed from logical reasoning without anyone observing the symptom; without an actual reproduction we can't tell «fix works» apart from «bug doesn't exist on our hardware». If reproduction failed or no hypothesis matched → escalate to user.
