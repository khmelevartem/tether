...
- **Code-track re-entry.** На текущей feature-branch могут быть новые комменты ревьюера или коммиты после прошлого прогона. На PR есть **две независимые ветки комментов** — обработай каждую отдельно, они не сливаются в один поток и не дублируют друг друга:
  1. **PR-level conversation** — top-level комменты на странице PR. Сюда уходят архитектурные / общие / cross-cutting замечания. Получи: `gh api repos/<owner>/<repo>/issues/<PR>/comments`.
  2. **Inline review threads** — привязаны к строкам diff'а, группируются в threads. Получи: `gh api repos/<owner>/<repo>/pulls/<PR>/comments`.

  Пропуск PR-level ветки — распространённая ошибка: inline-batch визуально перекрывает её, а именно там часто живут самые важные структурные вопросы. Для каждого коммента в обеих ветках определи статус: адресован в коммитах после него — или нет. **Дата создания не определяет актуальность** — фильтровать комменты по `created_at > <дата-прошлого-прогона>` запрещено, потому что неадресованный коммент остаётся актуальным независимо от того, насколько он старый. Прогон **обязан** включать на свежем diff'е: Step 5 (inner loop reviewers) → Step 6 (simplify) → Step 7 (full review wave A + adversarial) → Step 8 (smoke, скоуп по diff'у). Из дисциплины на re-entry ничего пропускать нельзя — иначе review-итерации проходят с меньшим качеством, чем первичная имплементация.
