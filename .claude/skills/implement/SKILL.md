...
1. Dispatch the implementing agent with the plan slice:
   ...
   **Prose discipline carry-forward.** When the plan slice includes prose edits (specs, KDocs, comments, READMEs, ADRs, `.claude/` prompts), the dispatch brief must instruct the implementing agent to load [`docs/engineering/long-lived-artifacts.md`](../../../docs/engineering/long-lived-artifacts.md) before writing, and to apply its rules — rule-not-history, link-over-inline-copy, refer-to-a-slot-functionally — to every paragraph it produces or edits. The Step 5 simplify wave already references this file, but loading it upstream means violations don't reach the diff in the first place.
