...
> **Do not rephrase prose for brevity.** If a sentence is load-bearing and free of the issues above, leave its wording alone — shaving words off well-formed sentences (`preference saved to suppress future dialog appearances` → `suppression preference saved`, dropping a leading `Note:`) inflates the diff between commits without changing meaning. Cut whole sentences when they fail the rule above; otherwise keep them as written.
