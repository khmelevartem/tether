...
**Exception — forced cascade outside literal scope.** When the expansion falls outside the issue's literal **Out of scope** *and* is forced (by DoD wording, by not-breaking-shipped-behaviour, or by getting verification infra working), the fold-vs-split decision belongs to the user — route through the `Forced-cascade scope expansion` gate (§Gate semantics), do not silently fold and do not present as a heads-up.
