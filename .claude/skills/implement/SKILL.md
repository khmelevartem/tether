...
- **BUGFIX root cause** — root cause must be confirmed before any fix. **Mitigation:** dispatch `bug-reproducer`; only stop at user if it reports CANNOT REPRODUCE or none of the listed hypotheses match. **The reproducer must always attempt to observe the symptom**, even when the cause looks structurally evident from issue text + code grep. Do not silently proceed as if the bug were confirmed. **Re-dispatch on direction change:** if a fix attempt gets reverted, or a new hypothesis emerges that materially differs from the one `bug-reproducer` confirmed (different mechanism / different code path / different platform scope), the new hypothesis must pass `bug-reproducer` again before code is written against it. The first dispatch covers the first hypothesis only; subsequent direction changes do not inherit its confirmation.
