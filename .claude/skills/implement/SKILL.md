...
>
> **Distinguish history from load-bearing forward constraint before deleting.** "Runtime snapshot, not rule" applies to *history of decision* (e.g. "after retro #N", "in this iteration we decided X"). It does NOT apply to *forward constraints* — statements telling the next implementer what to create, add, or verify because the codebase doesn't yet have it (e.g. "AndroidManifest declares ACCESS_WIFI_STATE but not ACCESS_NETWORK_STATE — must add"; "TBD — verify on real hardware before relying on"). These are actionable instructions disguised as runtime statements; deleting them looks like cleanup but ships an underspecified doc. Keep them, rephrase in active voice if needed ("the implementation must add ACCESS_NETWORK_STATE"), but do not drop the constraint.
>
