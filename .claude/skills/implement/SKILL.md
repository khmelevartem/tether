...
**Sweep the repo for prior mentions of `#<N>`.** Run `grep -rn "#<N>" --include='*.kt' --include='*.md' .` (and any other relevant extensions) over the working tree. Sibling code TODOs, stub functions, comments and docs left by earlier issues often pin scope to *this* issue — they must be addressed in this PR, not left dangling for a future task to discover. Treat every hit as either a deliverable (resolve it) or a deliberate carry-over (note it in the plan); never silently leave a `TODO(#<N>)` after merge.
