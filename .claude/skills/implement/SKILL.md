...
- **Forced-cascade scope expansion** — a change is required (by the issue's DoD wording; by the kernel otherwise visibly breaking or silently degrading shipped behaviour; or by existing verification infra being broken in a way that blocks the kernel's AC) but falls **outside the issue's literal Out of scope / Entry point**. The cascade being technically forced does not decide *which PR carries it*. Stop and present the fork: **fold** into this PR (kernel + cascade in one diff); **split** — narrow this PR to the literal kernel and file a follow-up issue for the cascade, accepting any temporary degradation; or **re-frame** the issue itself when the cascade exposes a DoD that no longer fits. Do not present the cascade as a heads-up or proceed unilaterally because the technical answer is clear — "what to do" being clear does not make "which PR" clear. The cost of one pause is small; the cost of unwinding a unilateral scope absorption (a PR a reviewer has to triage as two unrelated changes; a follow-up that never happens because the kernel already shipped without it) is large.
...
**Exception — forced cascade outside literal scope.** When the expansion falls outside the issue's literal **Out of scope** / **Entry point** *and* is forced (by DoD wording, by not-breaking-shipped-behaviour, or by getting verification infra working), the fold-vs-split decision belongs to the user — route through the `Forced-cascade scope expansion` gate (§Gate semantics), do not silently fold and do not present as a heads-up.
