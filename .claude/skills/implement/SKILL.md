...
- **G1. Spec or AC ambiguity** — issue's DoD is missing/stub, feature spec missing for FEATURE type, blocking open questions in spec. **Mitigation:** dispatch `spec-writer` first; only stop at user if `spec-writer` has clarifying questions or the issue is non-FEATURE without DoD.
  - **DOCS-as-decision sub-class.** A DOCS issue whose deliverable IS the decision (closing an open question, picking between options, ADR-style choice) qualifies as G1 by default — even when AC look complete («сравни 3 варианта, выбери один и обоснуй»). The *act of choosing* requires user input. Surface the comparison, recommend if you have signal, but do not commit without explicit OK.
    - **Pluralistic research before surfacing options.** Before presenting alternatives to the user, dispatch **≥3 research sub-agents in parallel with different prompts and different angles** (e.g. references / brand-recall / technical feasibility for a visual-identity decision). Convergence across independent angles is evidence of a robust choice; divergence is evidence of a real trade-off worth surfacing. A single research pass tends to rationalise into the agent's first hypothesis.
    - **Fact-check research-agent output before it becomes a locked artefact.** Sub-agents assert factual claims (library coordinates, API status, dependency availability, runtime behaviour) with confidence that exceeds verification. Before any such claim is committed to a spec / ADR / guide / agent file, the orchestrator verifies it directly — Maven Central solrsearch for package coordinates, jar/KLib inspection for runtime package paths, GitHub issue state for "is this bug fixed" claims, official docs for API surface. Treat sub-agent factual confidence as a hypothesis, not as evidence.
    - **DoD must include an enforcement artefact when the decision locks rules.** If the locked decision sets rules that future code/work must obey (e.g. "no Material 3", "this color is brand-only", "icons from this set"), the DoD has to include *how the rule is enforced going forward* — a reviewer agent, a lint rule, a pre-commit hook, a test. A decision without enforcement is a decision that quietly drifts. Surface this gap to the user during scoping if the issue body omits it.
