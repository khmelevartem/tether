...
3. Dispatch a **fast reviewer wave** in parallel:
   ...
   - `review-ux-conformance` (if diff touches `composeApp/src/**` — the agent itself decides skip vs. block on missing brief)
...
If anything was simplified — **commit the simplification** (see Step 4 discipline — reviewers read only the committed diff), then re-run **the same set of agents that ran in Step 4 for this PR type** (i.e. dod + guides + glossary + correctness + tests + platform-if-touched + ux-conformance-if-touched + ux-brief-if-touched + design-system-if-touched + visual-if-touched) **plus `review-reuse`** on the simplified diff. `review-reuse` is critical here because duplication is what most likely accumulated across iterations and tracks. `review-platform`, `review-ux-conformance`, `review-design-system`, and `review-visual` follow the same skip rules as Step 4 (platform set touched / `composeApp/src/**` touched); `review-ux-brief` follows its own (any `docs/product/features/**/ux-brief.md` touched). If clean, proceed.
...
2. Wave A in parallel: `review-dod`, `review-guides`, `review-glossary`, `review-reuse`, plus (if applicable to PR type / diff) `review-architecture`, `review-correctness`, `review-tests`, `review-platform`, `review-ux-conformance`, `review-ux-brief`, `review-design-system`, `review-visual`. Each agent receives the issue number and is told to review the local working tree (`git diff main...HEAD`) instead of a PR. `review-ux-conformance`, `review-design-system`, and `review-visual` all run whenever the diff touches `composeApp/src/**`; `review-ux-brief` runs whenever the diff touches any `docs/product/features/**/ux-brief.md`; each agent decides skip vs. block.
