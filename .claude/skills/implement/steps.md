...
## Step 15 — transform input to actions
...
## Step 16a — code-dispatch

**Applies to:** `track=code`

Dispatch every corresponding writing agent with a proper prompt:

- non-trivial mechanism / library / structural choice → `architect` first. When the issue prescribes a solution shape, withhold it on the first dispatch — let the architect design blind, then re-dispatch with the prescription to compare and synthesize.
- UI work (Compose, screens, components, theming, navigation) → `ui-expert`
- Everything else → `coder`

---

## Step 16b — docs-dispatch
