...
Each `##` section is one step. The `active-when:` line is the predicate `select-pipeline.sh` encodes — the model does not re-evaluate it. Steps are walked in the order the manifest names them; skipped steps are not walked.
...
The ordered step list produced here is fixed for this run — it depends only on `track`, `type`, and `reentry`. The reviewer roster is regenerated by `select-pipeline.sh` at each review step (Step 7, Step 10) from the live committed diff, because `touched` is only real once code is committed. The model never self-selects reviewers.

### Briefing to the user

After classify + recon (Step 2), post a short 3–6 line briefing before any gate question: what we are doing, why (motivation from the issue), track + affected layers/platforms. If asking questions — attach 1–2 lines of context to each so the user can answer without re-reading GitHub. One briefing per run; do not repeat on re-entry.

---

## Step 1 — reentry-reconcile

**active-when:** `reentry == pr-feedback`
...
**active-when:** `track == code` OR `reentry == fresh`
...
Mention relevant surfaced documents in the briefing to the user (Step 0).
...
If FEATURE and (no spec, or spec is `(stub)`, or spec has blocking open questions) → dispatch `spec-writer`. It drafts questions for the user or produces a scoped spec. Only escalate to the user with `spec-writer`'s question list.
...
**Recovery in Step 7.** If `ui-expert` halts reporting "UX brief missing" — re-dispatch `ux-expert` and resume. Machine-resolvable; do not escalate to the user.
...
After receiving a confirmed cause, show the paste-ready block and ask "Publish as a comment on issue #\<N\>?" Wait for explicit OK before `gh issue comment <N>`. Keep the confirmed cause locally as a hard constraint for the coder regardless of publication.
...
Decide which artifact layers this issue needs. The layer table is in Step 0. Read the issue body, comments, linked spec/feature (if any), and `docs/product/features/README.md` / `docs/engineering/README.md` for what already exists.

Multiple layers per issue are normal. **Ambiguity in classification → D1** (one question to the user before dispatching anything).
...
1. **spec** (if in `docLayers`) → dispatch `spec-writer`. It decides user needs and scenarios, runs its own clarifying-questions phase, scope cohesion pass, and `docs/product/features/README.md` row update. Open questions → D1 → relay verbatim → re-dispatch with answers.

2. **ux** AND **tech / ADR / knowledge** — if both needed, dispatch in parallel (file-disjoint by construction):
   - **ux** → dispatch `ux-expert`. Open UX questions → D1.
   - **tech / ADR / knowledge** → dispatch `architect`. For knowledge entries the design work was already done; architect records it in sibling-matching shape. Open questions → D1.
...
2. **Terminology consistent.** Same concept = same name across all touched artifacts. Variation = either mechanical rename or D2.
3. **Scope cohesion.** For each artifact, ask: does every section depend on the central invariant of this artifact's feature/subsystem? Sections describing concepts that survive without that invariant belong elsewhere. Mechanical move = do it; concept-level scope dispute = D2.
...
6. **Relocation completeness.** When the diff removes or moves a decision / section, verify: (a) it is homed somewhere in canon, not simply deleted; (b) every inbound link to the removed anchor is repointed (`grep -rn '<old-file>.md#<anchor>' docs/ *.md`). Missing home = D2; dangling link = mechanical fix, apply directly.

Mechanical fixes (rename, add missing link, missing index row) — apply directly. Conceptual fixes → D2.
...
If anything was simplified — commit, then re-review with a **delta set**: `review-reuse` always (duplication is what most likely accumulated — this is the gate's point), plus only reviewers whose domain the simplification touched. The full-roster pass is Step 10, immediately after.

---

## Step 10 — full-review

**active-when:** always (roster varies by track and type)

Pre-PR review wave, inline — no GitHub publication (the PR does not exist yet; Step 12 creates it).

1. Before Wave A — the working tree must be clean (committed). If uncommitted edits remain after Step 9 (code) or Step 8 (docs) — commit them. Reviewers read `git diff main...HEAD`.
...
Only after Step 11 is 🟢 (code track) or after Step 10 converges (docs track).
