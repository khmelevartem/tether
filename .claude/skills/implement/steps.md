...
**Worktree precondition.** Before dispatching any agent that writes files, ensure the working directory is `.claude/worktrees/<N>-<short-slug>/`. If missing, create from `origin/main` (branch name and worktree name share the same shape — no `feature/` or `docs/` prefix):
...
The ordered step list produced by `select-pipeline.sh` is fixed for this run — it depends only on `track`, `type`, and `reentry`. The reviewer roster is regenerated by `select-pipeline.sh` at each review step (`inner-loop`, `full-review`) from the live committed diff, because `touched` is only real once code is committed. The model never self-selects reviewers.
...
- **Recovery in `inner-loop`.** If `ui-expert` halts reporting "UX brief missing" → re-dispatch `ux-expert` and resume. Machine-resolvable; no user stop.
...
Selection from diff scope:

| Diff scope | Run |
|---|---|
| Common code touched (`commonMain` or shared sources) | All smoke blocks (Desktop, Android if device attached, iOS sim) |
| Single platform source set only | Smoke block for that platform |
| `docs/`-only / `.claude/`-only / comments-only | Nothing |

(Could be derived from `classify.sh`'s `touched` set in future; today the orchestrator reads the diff.)
...
Branches and worktrees share the same shape — `<N>-<short-slug>`, no `feature/` / `docs/` prefix.
