...
Writes to files inside the repo but outside the active worktree are blocked by a PreToolUse hook (`.claude/hooks/block-cross-worktree-writes.sh`); for a deliberate cross-worktree write set `TETHER_SKIP_WORKTREE_HOOK=1`. Its companion `block-stray-worktree.sh` denies Bash commands run from an unregistered `.claude/worktrees/` dir — one where git would fall through to the main checkout and corrupt it — until the worktree is rebuilt (`git worktree add`); the same `TETHER_SKIP_WORKTREE_HOOK=1` overrides it.
