...
После `git rebase` / `git merge` main прочитай заехавшие коммиты (`git log <prev>..origin/main`) и оцени смысловое пересечение с текущей работой. Чистый merge без конфликтов не означает отсутствие семантического столкновения.
