...
- `/document <N>` — docs-only оркестратор. Планирует слои артефактов (spec / ux-brief / tech-doc / ADR / `.claude` prompt), гоняет spec-writer / ux-expert / tech-writer, делает consistency pass, docs-scoped ревью, PR. Гейты D1/D2/D5. Используй напрямую для DOCS/INFRA задач или жди делегации из `/implement`.
