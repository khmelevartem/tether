...
- **Утверждения доков о runtime — снапшот, не правило.** Любое описание текущего поведения кода в долгоживущем артефакте (спека, doc, комментарий, скилл) — снапшот на момент написания и со временем устаревает. Опираешься на такое утверждение — сверь с кодом (`grep`/`Read`) до использования. Пишешь сам — предпочти продуктовый инвариант / правило («pairing keyed by stable device identity») описанию текущей реализации («pairing currently uses SHA-256 of publicKey»). Если без упоминания runtime не обойтись — оставь ровно столько, сколько критично для понимания, и формулируй так, чтобы устаревшее утверждение было видно при следующем чтении.
